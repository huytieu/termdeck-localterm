...
export interface ManagedClient {
  ...
  // didn't send one.
  windowId: string;
  // "Follow" mode (the grid tiles, `?follow=1`). A follow client is a passive
  // viewer: it never takes resize ownership (so a narrow tile can't clamp or
  // steal sizing from a full viewer of the same session — the "terminal
  // squeeze" bug) and only sizes the PTY as a fallback when it's the sole kind
  // of viewer attached (a grid-only session). See SessionClientHub.
  follow: boolean;
...
export class SessionManager {
  ...
  attach(
    ...
    const managed = this.clientHub.attach(ws, id, owner, windowId, follow);
