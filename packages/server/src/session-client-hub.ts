...
export class SessionClientHub {
  ...
  attach(
    ...
    } else if (!follow && managed.resizeOwner.follow) {
      // A real viewer arriving takes sizing over from a passive grid tile, so
      // a session first opened in the grid resizes to the full viewer at once.
      this.promoteResizeOwner(managed, client);
    } else {
  ...
  private promoteResizeOwner(managed: ManagedSession, client: ManagedClient): void {
    ...
    // A follow (grid-tile) client never steals sizing from a live full viewer;
    // it may own only when no non-follow client is attached (grid-only session).
    if (client.follow && managed.resizeOwner !== null && !managed.resizeOwner.follow) return;
  ...
  private latestClientByActivity(
    ...
    // Prefer non-follow (full) viewers; a passive grid tile inherits sizing
    // only when it's the last kind of viewer left.
    let latestClient: ManagedClient | null = null;
    let latestFollowClient: ManagedClient | null = null;
    for (const client of managed.clients) {
      if (focusedOnly && !client.focused) continue;
      if (client.follow) {
        if (
          latestFollowClient === null ||
          client.lastActivitySequence > latestFollowClient.lastActivitySequence
        ) {
          latestFollowClient = client;
        }
        continue;
      }
      if (
    ...
    return latestClient ?? latestFollowClient;
  }
