...
describe("Markdown", () => {
  it("uses theme-aware semantic colors throughout agent output", () => {
    ...
    // The TermDeck fork styles links with the coral primary token and inline
    // code with the muted pill (see makeComponents in markdown.tsx).
    expect(screen.getByText("link").className).toContain("text-primary");
    expect(screen.getByText("inline").className).toContain("text-foreground");
