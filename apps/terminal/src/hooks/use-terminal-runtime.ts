...
export const useTerminalRuntime = ({
  ...
  useEffect(() => {
    ...
    const terminalSurface = createTerminalSurface({
      ...
      // other previewable http(s) URL (a deploy link) opens in the drawer as a live
      // framed preview with the element picker. Both link back to THIS session so
      // "chat about this" / a tweak routes into it.
      openLink: (uri) => {
    ...
    // the wiki view (WebLinksAddon keeps handling real URLs). Resolve the token
    // to an absolute path against the live session cwd so the wiki pane (and
    // its iframe-tile bridge) can render it directly.
